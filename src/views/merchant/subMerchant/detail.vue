  ...
  <section class="page-container">
    <el-form :model="merchantForm" :rules="formRules" ref="formRuleRef"
      ...
      <div v-if="activeIndex === 0 || route.query.type === 'view'" style="margin-top: 20px">
        ...
        <el-form-item label="服务角色" prop="subMerchantRole">
          <el-select v-model="merchantForm.subMerchantRole" placeholder="请选择">
...
import {
  postDetailSubMerchantApi,
  postAddSubMerchantApi,
  postSubMerchantSaveApi
...
const formRules ={
  ...
  subMerchantRole: [{ required: true, message: '请选择服务角色', trigger: 'change' }],
...
onMounted(()=>{
  if(route.query && route.query.id){
    getDetailSubMerchant()
  }
})
const getDetailSubMerchant = () => {
  postDetailSubMerchantApi({
    merchantNum: route.query.id
  }).then(res => {
    if(res.code === 200) {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      if(activeIndex.value === 3) {
        if(handLicenceUrl.value){
          merchantForm.value.attachments.forEach(attachment => {
            if (attachment.fileType === 'handLicenceUrl') {
              attachment.filePath = handLicenceUrl.value
            }
            if(attachment.fileType === 'legalLicenceFrontUrl'){
              attachment.filePath = legalLicenceFrontUrl.value
            }
            if(attachment.fileType === 'legalLicenceBackUrl'){
              attachment.filePath = legalLicenceBackUrl.value
            }
            if(attachment.fileType === 'openAccountLicenceUrl'){
              attachment.filePath = openAccountLicenceUrl.value
            }
            if(attachment.fileType === 'screenshotOfPaymentProcessUrl') {
              attachment.filePath = screenshotOfPaymentProcessUrl.value
            }
            if(attachment.fileType === 'paymentScene'){
              attachment.filePath = paymentScene.value
            }
            if(attachment.fileType === 'handLicenceUrl'){
              attachment.filePath = handLicenceUrl.value
            }
            if(attachment.fileType === 'scenePhotoUrl'){
              attachment.filePath = scenePhotoUrl.value
            }
          })
        }
        merchantForm.value.areaCode = merchantForm.value.cityCode
        merchantForm.value.areaName = merchantForm.value.cityName
        postAddSubMerchantApi(merchantForm.value).then(res=>{
          if(res.code === 200) {
        ...
        postSubMerchantSaveApi({
          ...
          subMerchantRole: merchantForm.value.subMerchantRole,
